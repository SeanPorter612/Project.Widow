...
void LaneV2::SetKey ( char* csKey )
{
	char* csSeed;

	m_csKey = csKey;

	if ( WI_StrGetLength ( m_csKey ) > 1 )
	{
...
//		AfxMessageBox("WARNING: Missing Pass Phrase. Default in effect!");
	...
	for (nIdx = 0; WI_StrGetLength ( csSeed ) < 12; nIdx++)
	{
		csSeed[ nIdx ] = csSeed[ nIdx ];
	}
...
void LaneV2::TransformChar ( unsigned char& cTarget )
{
	int					Counter	= 0;
...
void LaneV2::TransformString( char* csKey, char* csTarget)
{
